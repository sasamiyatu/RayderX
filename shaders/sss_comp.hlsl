...
void cs_main( uint3 thread_id : SV_DispatchThreadID )
{
    ...
    float2 uv;
    {
        ...
        uv = (thread_id.xy + 0.5) / push_constants.resolution;
    }

    if (all(thread_id.xy < uint2(push_constants.resolution)))
    {
        //float4 in_color = in_render_target.SampleLevel(LinearSampler, uv, 0);
        float4 in_color = in_render_target[thread_id.xy];
        bool init_stencil = false;
        float4 out_color = SSSSBlurPS(thread_id.xy, uv, in_render_target, in_linear_depth, push_constants.sss_width, push_constants.dir, init_stencil);
